...
public class HomeController : Controller
{
    ...
    [HttpGet("generator")]
    public IActionResult Generator() //cshtml file was named Generate instead of Generator so renamed
    {
        if(HttpContext.Session.GetString("Username") == null) //the session is called "Name" here and "Username" in post above
        {
    ...
    {
        GeneratePasscode(); //call on function
        return RedirectToAction("Generator"); //this is only redirecting us to the file but not creating a new passcode (we created one called Generate Passcode belopw)
    }
